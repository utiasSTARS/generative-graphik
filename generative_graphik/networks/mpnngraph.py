...
class MPNNGraph(nn.Module):
    def __init__(
        ...
        mlp = nn.Sequential(
            ...
            non_linearity, 
            nn.Linear(edge_features_dim, (latent_dim + latent_dim) * (latent_dim + latent_dim))
        )
        self.gnn = nn.ModuleList()
        for _ in range(num_gnn_layers):
            self.gnn.append(
                NNConv(latent_dim + latent_dim, latent_dim + latent_dim, mlp)
            )
